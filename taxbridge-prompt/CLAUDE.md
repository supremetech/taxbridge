...
Hướng dẫn cho Claude Code khi làm việc trong `project/` và các repo con.
...
12/09/2026), demo 2 phút trên iPhone thật. Mục tiêu duy nhất: **chạy end-to-end 9 use
case ở mục 9**, code ngắn, dễ sửa. Không hardening, không production-ready.
...
```text
project/
├── CLAUDE.md → taxbridge-prompt/CLAUDE.md   (symlink; bản thật nằm trong git)
├── taxbridge-prompt/               git · master
│   ├── CLAUDE.md
│   ├── contract/                   README (quy ước/enum/error/Zalo) · endpoints.md · fixtures/*.json
│   ├── feature-map/                kịch bản kiểm chứng 2 luồng hero: handle UI + lệnh + bằng chứng (§3.4)
│   ├── implement-plan-backend-poc.md
│   ├── implement-plan-flutter-poc.md
...
│   └── demo-assets/                sale_voice.{m4a,aac} + receipt.jpg · transfer_match.jpg · transfer_deposit.jpg
├── taxbridge-server/               git · supremetech/taxbridge-server · branch poc-new
│   └── functions/                  Python 3.12 · Flask · Firebase Functions gen2
└── taxbridge-app/                  git · supremetech/taxbridge-app · branch poc-new
    └── lib/                        Flutter · Riverpod 3 · go_router · Dio
```

Ba repo git độc lập; `project/` không phải git repo — `cd` đúng repo trước khi chạy git.
...
2. Plan backend: mục 2 (Firestore), 4 (capture/AI), 6–8 (movement, dashboard, close day),
   9 (Zalo).
3. Plan Flutter: mục 3 (session/API), 6 (màn hình, hero), 7 (state).
...
**Làm:** register/login (token) · text/voice/ảnh capture → AI draft · sửa/confirm draft ·
...
audit trail, ledger double-entry, offline sync, retry/backoff, duplicate detection,
prompt-injection hardening, mask/redact, realtime Firestore ở app, App Check, Crashlytics,
dark mode, i18n, test coverage lớn, xử lý lại message Zalo cũ trước khi link.
...
- Capture xử lý **đồng bộ**, trả `{captureId, status, resultType, resultId, error}`;
  AI lỗi → HTTP 200 + `status: FAILED`, không 5xx.

Enum canonical (không tự thêm value ở bất kỳ bên nào):

```text
Capture.type:          TEXT | AUDIO | IMAGE_RECEIPT | IMAGE_TRANSFER   (+ IMAGE_UNKNOWN nội bộ backend, ảnh Zalo)
...
captureType:           TEXT | AUDIO | IMAGE_RECEIPT | IMAGE_TRANSFER (event + movement)
```
...
- AI: `SALE` + "chuyển khoản" → `paymentMethod=BANK`, `paymentStatus=UNPAID`; chỉ `PAID`
  ...
  hiển thị là chủ shop → `counterparty` lấy từ nội dung CK. `occurredAt` của event/movement
  = now(), không đọc ngày trên ảnh. Đủ rule (đánh dấu *eval*) ở plan BE §4.1.
...
- Close day: upsert `daily_records/{date}` đúng shape DTO (nested `summary`). Sau mọi
  confirm/reject/match/classify: warning OPEN trỏ tới `resourceId` → `RESOLVED` +
  `resolvedAt`, recompute `summary`.
- Zalo Bot Platform: payload, media URL public, voice `.aac` → remux `.m4a`
  (`imageio-ffmpeg`), body rỗng → 200, secret header, luôn trả
  200 — tất cả ở `contract/README.md` §6. Chưa link → `zalo_unlinked_messages`; đã link →
  capture `source=ZALO`, ảnh = `IMAGE_UNKNOWN` (prompt trả `kind`).
...
- Home: 4 stat card + dòng **Tiền vào ngân hàng** + dòng **Thuế khoán ước tính**
  (`revenue × 1,5%`, tính ở client) + badge `draftCount` (Giao dịch) / `unmatchedMoneyCount`
  (Tiền vào). Sau confirm/match/classify: invalidate rồi `context.go('/home')`; Home hiện
  delta banner 4 s (`Tiền vào +380.000đ · Doanh thu không đổi ✓`) và tự refresh mỗi 8 s.
